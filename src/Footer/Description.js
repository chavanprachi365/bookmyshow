...
const data =[
    {
        title:"Enjoy Online Ticket Booking for Movies in aurangabad With BookMyShow",
    ...
    {
        title:"Latest Movies To Watch in aurangabad With Family And Friends",
  ...
  return (
    <section className='container my-3'>
        {data.map((value,index)=>{
            return   <div key={index} className='text-muted  '>
            <h6 className='p_h14 '>{value.title}</h6>
            <p className='p_h12 mt-2 fw-light'>{value.Description}</p>
        </div>
        })}
    </section>
  )
